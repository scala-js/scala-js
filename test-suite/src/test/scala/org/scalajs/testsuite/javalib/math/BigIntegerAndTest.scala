/*
 *  Ported by Alistair Johnson from https://github.com/gwtproject/gwt/blob/master/user/test/com/google/gwt/emultest/java/math/BigIntegerAndTest.java
 */
...
object BigIntegerAndTest extends JasmineTest {

  describe("BigIntegerAndTest") {
    ...
    it("testOneOne") {
      ...
      val result = aNumber.and(bNumber)
      expect(result == BigInteger.ONE).toBeTruthy()
      expect(result.signum()).toEqual(1)
    }
    ...
    it("testZeroOne") {
      ...
      val bNumber = BigInteger.ONE
      val result = aNumber.and(bNumber)
      expect(result == BigInteger.ZERO).toBeTruthy()
      expect(result.signum()).toEqual(0)
    }
