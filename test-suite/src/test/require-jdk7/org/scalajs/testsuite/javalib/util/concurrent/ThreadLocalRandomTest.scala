...
object ThreadLocalRandomTest extends JasmineTest with ExpectExceptions {
  ...
  describe("java.util.concurrent.ThreadLocalRandom") {

    it("should throw exception on setSeed") {
      val tlr = ThreadLocalRandom.current()

      expectThrows[UnsupportedOperationException](tlr.setSeed(1))
    }
    ...
    def checkIntBounds(b1: Int, b2: Int)(implicit tlr: ThreadLocalRandom): Unit = {
      ...
      var next = tlr.nextInt(least, bound)

      expect((next >= least) && (next < bound))
    }

    it("should return nextInt that fits bounds") {
      ...
      expectThrows[IllegalArgumentException](tlr.nextInt(2, 1))
      expectThrows[IllegalArgumentException](tlr.nextInt(1, 1))
    }

    def checkLongUpperBound(bound: Long)(implicit tlr: ThreadLocalRandom): Unit = {
      var next = tlr.nextLong(bound)

      expect(next < bound)
    }

    it("should return nextLong under a bound") {
      implicit val tlr = ThreadLocalRandom.current()
      ...
      expectThrows[IllegalArgumentException](tlr.nextLong(0L))
      expectThrows[IllegalArgumentException](tlr.nextLong(-1L))
      expectThrows[IllegalArgumentException](tlr.nextLong(Long.MinValue))
    }
    ...
    def checkLongBounds(b1: Long, b2: Long)(implicit tlr: ThreadLocalRandom): Unit = {
      ...
      var next = tlr.nextLong(least, bound)

      expect((next >= least) && (next < bound))
    }

    it("should return nextLong that fits bounds") {
      implicit val tlr = ThreadLocalRandom.current()
      ...
      expectThrows[IllegalArgumentException](tlr.nextLong(2L, 1L))
      expectThrows[IllegalArgumentException](tlr.nextLong(1L, 1L))
    }

    def checkDoubleUpperBound(bound: Double)(implicit tlr: ThreadLocalRandom): Unit = {
      var next = tlr.nextDouble(bound)

      expect(next < bound)
    }

    it("should return nextDouble under a bound") {
      implicit val tlr = ThreadLocalRandom.current()
      ...
      expectThrows[IllegalArgumentException](tlr.nextDouble(0.0))
      expectThrows[IllegalArgumentException](tlr.nextDouble(-1.0))
      expectThrows[IllegalArgumentException](tlr.nextDouble(Double.MinValue))
    }
    ...
    def checkDoubleBounds(b1: Double, b2: Double)(implicit tlr: ThreadLocalRandom): Unit = {
      ...
      var next = tlr.nextDouble(least, bound)

      expect((next >= least) && (next < bound))
    }

    it("should return nextDouble that fits bounds") {
      implicit val tlr = ThreadLocalRandom.current()
      ...
      expectThrows[IllegalArgumentException](tlr.nextDouble(2.0, 1.0))
      expectThrows[IllegalArgumentException](tlr.nextDouble(1.0, 1.0))
      expectThrows[IllegalArgumentException](tlr.nextDouble(0.0, 0.0))
    }
  }
}
