...
class UTF8Test extends BaseCharsetTest(Charset.forName("UTF-8")) {
  ...
  @Test def decodeLonelyStartCharacters(): Unit = {
    ...
    val vc = (0xc0 to 0xf4).flatMap(_ =>
      List[OutPart[CharBuffer]](Malformed(1), cb" "))
