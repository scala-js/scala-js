...
class BitSetTest {
  ...
  @Test def valueOf_ByteBuffer(): Unit = {
    ...
    assertEquals(emptyBS, BitSet.valueOf(ByteBuffer.wrap(emptyBS.toByteArray())))

    val eightBS = makeEightBS()
    assertEquals(eightBS, BitSet.valueOf(ByteBuffer.wrap(eightBS.toByteArray())))
  }

  @Test def valueOf_LongBuffer(): Unit = {
    val emptyBS = new BitSet
    assertEquals(emptyBS, BitSet.valueOf(emptyBS.toLongArray()))

    val eightBS = makeEightBS()
    assertEquals(eightBS, BitSet.valueOf(eightBS.toLongArray()))
  }
