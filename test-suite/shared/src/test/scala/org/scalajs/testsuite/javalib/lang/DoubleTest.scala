...
class DoubleTest {
  ...
  @Test def should_parse_strings(): Unit = {
    ...
    for (padding <- List("", "  ", (0 to 0x20).map(x => x.toChar).mkString)) {
      ...
      def testFull(s: String, v: Double): Unit = {
        ...
        assertTrue(s"""Double.parseDouble("$s2") must be $v, was $r1""", r1.equals(v))
        val r2 = JDouble.valueOf(s2)
        assertTrue(s"""Double.valueOf("$s2") must be $v, was $r2""", r2.equals(JDouble.valueOf(v)))
        val r3 = new JDouble(s2)
        assertTrue(s"""new Double("$s2") must be $v, was $r3""", r3.equals(JDouble.valueOf(v)))
      }
    ...
    def test(s: String, v: Double): Unit = {
      ...
      assertTrue(s"""Double.parseDouble("$s") must be $v, was $r""", r.equals(v))
    }
    ...
    test("0xad82fedb784e34be1b5bf0dc5cca18bdbba36311c1c6636e.fbd56a6f30cf3c943e89ecp8322",
        Double.PositiveInfinity)
    test(
        "0x6328d473cc9bf5bae48aa4e37e3e2660865cdf8d.a9c05efad8b3ddf34134efce48921ffc19b31f4f2bee3p-428",
    ...
    test(
        "0x783e5869aea0daffecb432b6de1bc40226c3568fd2edc5139.7341a4d9d452e7c8345b023477e6d05919f7689bba1e9a7p-6",
    ...
    test(
        "0x322be50cbd1f8130319a61db6d004164f288673bc13c0305.3a77e1de3e9ab9870e8810838fc52a3a1fp-22",
    ...
    test(
        "0xd4e9c94d3ddda786d3f154698d2360fffa8253c805cb.3e75dcf8a2cbb9bd556bbf81c223b3531b9098416f6p-9370",
        0.0)
  ...
  @Test def longBitsToDouble(): Unit = {
    def isZero(v: Double, neg: Boolean): Boolean = {
      (v == 0.0) && (1 / v == (if (neg) Double.NegativeInfinity
    ...
    assertEquals(-1.8790766677624813e63, f(0xcd124568bc6584caL),
        0.0) // an arbitrary neg normal form

    // Subnormal forms
    assertEquals(Double.MinPositiveValue, f(0x0000000000000001L),
        0.0) // smallest pos subnormal form
    assertEquals(2.225073858507201e-308, f(0x000fffffffffffffL), 0.0) // largest pos subnormal form
    assertEquals(1.719471609939382e-308, f(0x000c5d44ae45cb60L),
        0.0) // an arbitrary pos subnormal form
    assertEquals(-Double.MinPositiveValue, f(0x8000000000000001L),
        0.0) // smallest neg subnormal form
    assertEquals(-2.225073858507201e-308, f(0x800fffffffffffffL), 0.0) // largest neg subnormal form
    assertEquals(-1.719471609939382e-308, f(0x800c5d44ae45cb60L),
        0.0) // an arbitrary neg subnormal form
  }
  ...
  @Test def testStaticHashCode(): Unit = {
    assumeFalse("Hash codes for doubles are different in JS than on the JVM", executingInJVM)
