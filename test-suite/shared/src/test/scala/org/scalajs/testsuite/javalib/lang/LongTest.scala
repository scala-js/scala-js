...
class LongTest {
  ...
  @Test def divideUnsigned(): Unit = {
    @inline def test(x: Long, y: Long, result: Long): Unit = {
      ...
      assertEquals(
          result, JLong.divideUnsigned(hideFromOptimizer(x), hideFromOptimizer(y)))
    }
  ...
  @Test def remainderUnsigned(): Unit = {
    @inline def test(x: Long, y: Long, result: Long): Unit = {
      ...
      assertEquals(result,
          JLong.remainderUnsigned(hideFromOptimizer(x), hideFromOptimizer(y)))
    }
  ...
  @Test def divideRemainderUnsignedHugeDivisors(): Unit = {
    @inline def test(x: Long, y: Long, expectedQuot: Long,
        expectedRem: Long): Unit = {
      ...
      assertEquals(expectedQuot,
          JLong.divideUnsigned(hideFromOptimizer(x), hideFromOptimizer(y)))
  ...
  @Test def toUnsignedString(): Unit = {
    ...
    test(-111L,
        "1111111111111111111111111111111111111111111111111111111110010001", 2)
    test(-841L,
        "1111111111111111111111111111111111111111111111111111110010110111", 2)
    test(-48L,
        "1111111111111111111111111111111111111111111111111111111111010000", 2)
    test(11568553533L, "1010110001100010100001111000111101", 2)
    test(1448703278415412L,
        "101001001011001011010000100010010110111111000110100", 2)
    test(-78600467092795L,
        "1111111111111111101110001000001101100111000101000110101011000101", 2)
    test(97785243187L, "1011011000100011101000110011000110011", 2)
    test(-16595422816873L,
        "1111111111111111111100001110100000010011101101111101010110010111", 2)
    test(-8L, "1111111111111111111111111111111111111111111111111111111111111000",
        2)
    test(1423166986L, "1010100110100111100111000001010", 2)
    test(528027512103548939L,
        "11101010011111011100011100110000010100100001011100000001011", 2)
    test(581852302953L, "1000011101111001000110011000111001101001", 2)
    test(-1L, "1111111111111111111111111111111111111111111111111111111111111111",
        2)
    test(-1117567308L,
        "1111111111111111111111111111111110111101011000110100011010110100", 2)
    test(-10067395877878416L,
        "1111111111011100001110111100000110111100111000101100010101110000", 2)
    test(-3070582626L,
        "1111111111111111111111111111111101001000111110101010000010011110", 2)
    test(220037L, "110101101110000101", 2)
    test(-952L,
        "1111111111111111111111111111111111111111111111111111110001001000", 2)
    test(9266140931L, "1000101000010011100001011100000011", 2)
    test(-1356784282352L,
        "1111111111111111111111101100010000011001010110101101010100010000", 2)
  ...
  @Test def min(): Unit = {
    ...
    assertEquals(
        693040957319768557L, JLong.min(737425499269142100L, 693040957319768557L))
