...
class BaseCharsetTest(val charset: Charset) {
  ...
  protected def testDecode(in: ByteBuffer)(
    ...
    def prepareInputBuffer(readOnly: Boolean): ByteBuffer = {
      val buf = if (readOnly) in.asReadOnlyBuffer()
                else in.duplicate()
    ...
    def testDecodeVsSteppedDecode(malformedAction: CodingErrorAction,
      ...
      //compares the decode result of a single decoder.decode(buffer) call vs
      //repeated decode(input, output, done) calls with a buffer that increments
      //its limit each step
      //this will catch mishandled broken up multi-byte sequences

      val decoder = newDecoder(malformedAction, unmappableAction)

      val directResult = Try(decoder.decode(prepareInputBuffer(readOnly)))
                         .map(_.toString)
      ...
      def increaseBuffer(bb: ByteBuffer) = (bb.limit < bb.capacity) &&
                                           (bb.limit(bb.limit + 1) != null)
      ...
      (directResult, incrementalResult) match {
        case (Success(directString), Success(incrementalString)) =>
          assertSame(directString, incrementalString)
        case (Failure(dex: UnmappableCharacterException),
              Failure(incex: UnmappableCharacterException)) =>
          assertEquals(dex.getInputLength, incex.getInputLength)
        case (Failure(dex: MalformedInputException),
              Failure(incex: MalformedInputException)) =>
          assertEquals(dex.getInputLength, incex.getInputLength)
        //the fallback will assert false, but have an informational failure message
        case _ => assertSame(directResult, incrementalResult)
      }
