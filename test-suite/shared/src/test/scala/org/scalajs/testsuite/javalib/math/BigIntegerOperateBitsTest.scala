...
class BigIntegerOperateBitsTest {
  ...
  @Test def testFlipBitPositiveOutside1(): Unit = {
    ...
    val rBytes = Array[Byte](
        64, 0, 0, 0, 0, 0, 1, -128, 56, 100, -2, -76, 89, 45, 91, 3, -15, 35, 26)
    val aNumber = new BigInteger(aSign, aBytes)
    val result = aNumber.flipBit(number)
    var resBytes = Array.ofDim[Byte](rBytes.length)
    resBytes = result.toByteArray()
  ...
  @Test def testShiftsWithLargeCounts_Issue4870(): Unit = {
    val aNumber =
      new BigInteger(-1, Array[Byte](0, -82, 127, 15, 76, -97, 13, 30, 30))
    ...
    assertThrows(
        classOf[ArithmeticException], aNumber.shiftRight(Int.MinValue + 1))
