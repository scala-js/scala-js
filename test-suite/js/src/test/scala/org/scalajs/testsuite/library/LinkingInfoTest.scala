...
class LinkingInfoTest {
  ...
  @Test def assumingES6(): Unit =
    assertEquals(
        Platform.assumedESVersion >= ESVersion.ES2015, LinkingInfo.assumingES6)

  @Test def useECMAScript2015Semantics(): Unit =
    assertEquals(Platform.useECMAScript2015Semantics,
        LinkingInfo.useECMAScript2015Semantics)
