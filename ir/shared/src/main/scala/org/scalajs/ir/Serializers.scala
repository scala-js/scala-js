  ...
  private final class Deserializer(buf: ByteBuffer) {
      ...
      (tag: @switch) match {
        ...
        case TagIsInstanceOf =>
          ...
          val testType = if (hacks.useBelow(17)) {
            testType0 match {
              case ClassType(className, true, false)    => ClassType(className, nullable = false, exact = false)
              case ArrayType(arrayTypeRef, true, false) => ArrayType(arrayTypeRef, nullable = false, exact = false)
        ...
        case New(cls, _, funArg :: Nil) =>
          ...
          cls match {
            case HackNames.AnonFunctionClass(arity) =>
              ...
              NewLambda(HackNames.anonFunctionDescriptors(arity), typedClosure)(tree.tpe.toNonExact)(tree.pos)
            ...
            case HackNames.AnonFunctionXXLClass =>
              ...
              NewLambda(HackNames.anonFunctionXXLDescriptor, typedClosure)(tree.tpe.toNonExact)(tree.pos)
    ...
    private def jlReflectArrayMethodsHackBelow17(methods: List[MethodDef]): List[MethodDef] = {
      ...
      val newInstanceRecMethod = {
        ...
        val dimensions = paramDef("dimensions", ArrayType(intArrayTypeRef, nullable = true, exact = false))
                ...
                While(BinaryOp(BinaryOp.Int_!=, i.ref, arrayLength(lengthsParam.ref)), {
                  Block(
                    Assign(
                      ...
                      getClass(Apply(EAF, This()(ClassType(ReflectArrayModClass, nullable = false, exact = false)),
    ...
    def readType(): Type = {
      ...
      (tag: @switch) match {
        ...
        case TagArrayType             => ArrayType(readArrayTypeRef(), nullable = true, exact = false)
        case TagNonNullArrayType      => ArrayType(readArrayTypeRef(), nullable = false, exact = false)
        case TagExactArrayType        => ArrayType(readArrayTypeRef(), nullable = true, exact = true)
        case TagExactNonNullArrayType => ArrayType(readArrayTypeRef(), nullable = false, exact = true)
