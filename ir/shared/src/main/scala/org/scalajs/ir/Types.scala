...
object Types {
   ...
   *  of the element type.
   */
  final case class ArrayType(arrayTypeRef: ArrayTypeRef, nullable: Boolean, exact: Boolean) extends Type {
  ...
  def isSubtype(lhs: Type, rhs: Type)(
    ...
    ((lhs, rhs) match {
      ...
      case (ClassType(lhsClass, lhsNullable, lhsExact), ClassType(rhsClass, rhsNullable, rhsExact)) =>
        isSubnullable(lhsNullable, rhsNullable) && {
          if (rhsExact)
