 ...
 *
 *  The main difference compared to using individual files (that are extracted
 *  beforehand) is that the fileset can be versioned at a higher level: the
 *  container needs to change its version when any of the files change.
 *  Therefore, the entire extraction process can be cached.
 */
abstract class IRContainerImpl(
     ...
     *  not mandatory. Such a token can be used by caches: the file need not be
     *  read and processed again if its version has not changed.
     */
