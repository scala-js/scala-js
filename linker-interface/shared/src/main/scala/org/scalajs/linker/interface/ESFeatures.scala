 ...
 *  from `esVersion`. In the future, it might become independently configurable.
 */
final class ESFeatures private (
   ...
   *
   *    - provide more features that rely on recent ECMAScript language
   *      features, and/or
   ...
   *  to improve expected performance. Setting it to `false` provides a hint not
   *  to avoid `class`'es. Either way, the linker is free to ignore this option.
   *
   ...
   *  `class`'es, it is desirable to set this option to `false`. If the code is
   *  targeted at browsers (among others), it is recommended to set it to
   *  `true`.
   *
   *  This option never affects the code emitted for JavaScript classes (classes
   *  extending `js.Any`), since that would have an impact on observable
   *  semantics.
   *
   ...
   *  help readability and debugging, but there is little to no benefit in using
   *  them when the code is compiler-generated.
   *
   *  This option is always ignored when `esVersion < ESVersion.ES2015`.
   */
