...
abstract class Linker private[interface] () {
  ...
  final def link(irFiles: Seq[IRFile], moduleInitializers: Seq[ModuleInitializer],
      output: LinkerOutput, logger: Logger)(implicit ec: ExecutionContext): Future[Unit] = {
    ...
    link(irFiles, moduleInitializers, outDir, logger)
      ...
      .recover { case e: ReportToLinkerOutputAdapter.UnsupportedLinkerOutputException =>
...
private object Linker {
  ...
  private final class MemOutputDirectory extends OutputDirectoryImpl {
    ...
    def readFull(name: String)(implicit ec: ExecutionContext): Future[ByteBuffer] = synchronized {
