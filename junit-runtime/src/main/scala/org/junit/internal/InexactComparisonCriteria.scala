...
class InexactComparisonCriteria private (val fDelta: AnyRef) extends ComparisonCriteria {
  ...
  override protected def assertElementsEqual(expected: AnyRef, actual: AnyRef): Unit = {
    Assert.assertEquals(expected.asInstanceOf[Double], actual.asInstanceOf[Double],
