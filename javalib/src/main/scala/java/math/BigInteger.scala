...
object BigInteger {
  ...
  private final val SMALL_VALUES = Array(ZERO, ONE, new BigInteger(1, 2), new BigInteger(1, 3),
  ...
  private[math] def divideAndRemainderImpl(divisor: BigInteger): QuotAndRem = {
    ...
    if (divisorLen == 1) {
      ...
      if (cmp < 0) {
        ...
        val remainderDigits = Division.divide(quotientDigits, quotientLength, thisDigits, thisLen,
