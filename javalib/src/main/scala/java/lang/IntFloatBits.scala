 ...
 *  {{{
 *  @inline
 *  def genericAlgo[I, F](x: I)(implicit ops: IntFloatBits[I, F]): I = {
 *    import ops._
 *
 *    // ...
 *  }
 ...
 *
 *  There is one exception: avoid declaring `var`s of type `I`. They will be
 *  boxed. Instead, declare a `val` obtained with `ops.newIntBox`. This gives
 *  you a concrete instance of an inlineable class that has the correct,
 *  monomorphic type of variable inside. Paradoxically, this allows the
 *  optimizer to completely remove the box, unlike a `var` of a generic type.
 */
sealed abstract class IntFloatBits[I, F] {
...
object IntFloatBits {
  ...
  implicit object Bits32 extends IntFloatBits[Int, scala.Float] {
    ...
    @inline def fnzero: FloatType = -0.0f
    ...
    @inline def fninf: FloatType = scala.Float.NegativeInfinity
    ...
    @inline def unsigned_<(x: IntType, y: IntType): scala.Boolean = (x ^ SignBit) < (y ^ SignBit)
    @inline def unsigned_<=(x: IntType, y: IntType): scala.Boolean = (x ^ SignBit) <= (y ^ SignBit)
    @inline def unsigned_>(x: IntType, y: IntType): scala.Boolean = (x ^ SignBit) > (y ^ SignBit)
    @inline def unsigned_>=(x: IntType, y: IntType): scala.Boolean = (x ^ SignBit) >= (y ^ SignBit)
  ...
  implicit object Bits64 extends IntFloatBits[scala.Long, scala.Double] {
    ...
    @inline def fnzero: FloatType = -0.0
    ...
    @inline def fninf: FloatType = scala.Double.NegativeInfinity
