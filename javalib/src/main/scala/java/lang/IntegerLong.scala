...
private[lang] object IntegerLong {
  ...
  @inline
  private def parseFail[I, F](s: String)(implicit ops: IntFloatBits[I, F]): Nothing = {
    // This is not great, but it allows IntegerLong to be entirely dce'ed
    if (ops.bitSize == 32)
      Integer.parseIntFail(s)
    else
      Long.parseLongFail(s)
  }
