...
abstract class Reader private[this] (_lock: Option[Object]) extends Readable with Closeable {
  ...
  def read(target: CharBuffer): Int = {
    if (!target.hasRemaining()) 0
      ...
      val charsRead =
        read(target.array(), target.position() + target.arrayOffset(), target.remaining())
      if (charsRead != -1)
