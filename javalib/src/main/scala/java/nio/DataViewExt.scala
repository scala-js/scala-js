 ...
 *
 *  Defined as functions instead of extension methods, because the AnyVal over a
 *  JS type generates an `equals` method that references `BoxesRunTime`.
 */
private[nio] object DataViewExt {
   ...
   *    Starting index
   *  @param littleEndian
   *    Whether the number is stored in little endian
   */
  @inline
  def dataViewGetInt64(dataView: DataView, index: Int,
      littleEndian: Boolean): Long = {
    val high =
   ...
   *    Value to be written
   *  @param littleEndian
   *    Whether to store the number in little endian
   */
  @inline
  def dataViewSetInt64(dataView: DataView, index: Int, value: Long,
      littleEndian: Boolean): Unit = {
