...
private[nio] final class DataViewCharBuffer private (override private[nio] val _dataView: DataView,
  ...
  def subSequence(start: Int, end: Int): CharBuffer = {
    ...
    new DataViewCharBuffer(_dataView, position() + start, position() + end, isReadOnly(),
        isBigEndian)
  }
...
private[nio] object DataViewCharBuffer {
  private[nio] implicit object NewDataViewCharBuffer
    ...
    def apply(dataView: DataView, initialPosition: Int, initialLimit: Int, readOnly: Boolean,
      ...
      new DataViewCharBuffer(dataView, initialPosition, initialLimit, readOnly, isBigEndian)
    }
  }
