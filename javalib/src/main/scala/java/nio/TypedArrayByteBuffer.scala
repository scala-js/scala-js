...
private[nio] final class TypedArrayByteBuffer private (
    override private[nio] val _typedArray: Int8Array, _initialPosition: Int, _initialLimit: Int,
    _readOnly: Boolean)
...
private[nio] object TypedArrayByteBuffer {
  private[nio] implicit object NewTypedArrayByteBuffer
    ...
    def apply(typedArray: Int8Array, initialPosition: Int, initialLimit: Int,
        readOnly: Boolean): TypedArrayByteBuffer = {
      new TypedArrayByteBuffer(typedArray, initialPosition, initialLimit, readOnly)
    }

    @inline
    def newTypedArray(buffer: ArrayBuffer, byteOffset: Int, length: Int): Int8Array = {
