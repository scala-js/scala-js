...
private[nio] final class StringCharBuffer private (_capacity: Int, _csq: CharSequence,
    _csqOffset: Int, _initialPosition: Int, _initialLimit: Int)
  ...
  def duplicate(): CharBuffer = {
    val result = new StringCharBuffer(capacity(), _csq, _csqOffset, position(), limit())
  ...
  def subSequence(start: Int, end: Int): CharBuffer = {
    ...
    new StringCharBuffer(capacity(), _csq, _csqOffset, position() + start, position() + end)
  }
...
private[nio] object StringCharBuffer {
  private[nio] def wrap(csq: CharSequence, csqOffset: Int, capacity: Int, initialPosition: Int,
