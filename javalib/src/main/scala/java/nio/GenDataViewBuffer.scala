...
private[nio] object GenDataViewBuffer {
  ...
  trait NewDataViewBuffer[BufferType <: Buffer] {
    ...
    def apply(dataView: DataView, initialPosition: Int, initialLimit: Int, readOnly: Boolean,
        isBigEndian: Boolean): BufferType
  }

  @inline
  def generic_fromTypedArrayByteBuffer[BufferType <: Buffer](byteBuffer: TypedArrayByteBuffer)(
    ...
    val dataView = newDataView(byteArray.buffer, byteArray.byteOffset + byteBufferPos, byteLength)
    newDataViewBuffer(dataView, 0, viewCapacity, byteBuffer.isReadOnly(), byteBuffer.isBigEndian)
  }
...
private[nio] final class GenDataViewBuffer[B <: Buffer] private (val self: B) extends AnyVal {
  ...
  @inline
  def generic_slice()(implicit newDataViewBuffer: NewThisDataViewBuffer): BufferType = {
    val bytesPerElem = newDataViewBuffer.bytesPerElem
    val dataView = _dataView
    val pos = position()
    val newCapacity = limit() - pos
    val slicedDataView = newDataView(dataView.buffer, dataView.byteOffset + bytesPerElem * pos,
        bytesPerElem * newCapacity)
    newDataViewBuffer(slicedDataView, 0, newCapacity, isReadOnly(), isBigEndian)
  }

  @inline
  def generic_duplicate()(implicit newDataViewBuffer: NewThisDataViewBuffer): BufferType = {
    val result = newDataViewBuffer(_dataView, position(), limit(), isReadOnly(), isBigEndian)
    result._mark = _mark
    result
  }

  @inline
  def generic_asReadOnlyBuffer()(implicit newDataViewBuffer: NewThisDataViewBuffer): BufferType = {
    val result = newDataViewBuffer(_dataView, position(), limit(), true, isBigEndian)
    result._mark = _mark
    result
  }

  @inline
  def generic_compact()(implicit newDataViewBuffer: NewThisDataViewBuffer): BufferType = {
    if (isReadOnly())
      throw new ReadOnlyBufferException

    val dataView = _dataView
    val bytesPerElem = newDataViewBuffer.bytesPerElem
    val byteArray = new Int8Array(dataView.buffer, dataView.byteOffset, dataView.byteLength)
    val pos = position()
    val lim = limit()
