...
private[nio] final class HeapCharBuffer private (_capacity: Int, _array0: Array[Char],
  ...
  def subSequence(start: Int, end: Int): CharBuffer = {
    ...
    new HeapCharBuffer(capacity(), _array, _arrayOffset, position() + start, position() + end,
        isReadOnly())
  }
...
private[nio] object HeapCharBuffer {
  private[nio] implicit object NewHeapCharBuffer
    ...
    def apply(capacity: Int, array: Array[Char], arrayOffset: Int, initialPosition: Int,
        initialLimit: Int, readOnly: Boolean): CharBuffer = {
      new HeapCharBuffer(capacity, array, arrayOffset, initialPosition, initialLimit, readOnly)
    }
  }

  private[nio] def wrap(array: Array[Char], arrayOffset: Int, capacity: Int, initialPosition: Int,
      initialLength: Int, isReadOnly: Boolean): CharBuffer = {
