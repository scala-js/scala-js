...
final class URI(origStr: String) extends Serializable with Comparable[URI] {
  ...
  def this(scheme: String, userInfo: String, host: String, port: Int, path: String, query: String,
      fragment: String) = {
  ...
  def resolve(uri: URI): URI = {
    if (uri.isAbsolute() || this.isOpaque()) uri
      ...
      new URI(this.getScheme(), this.getRawAuthority(), this.getRawPath(), this.getRawQuery(),
          uri.getRawFragment())
    else if (uri._authority.isDefined)
      new URI(this.getScheme(), uri.getRawAuthority(), uri.getRawPath(), uri.getRawQuery(),
          uri.getRawFragment())
    else if (uri._path.get.startsWith("/"))
      new URI(this.getScheme(), this.getRawAuthority(), uri.getRawPath(), uri.getRawQuery(),
          uri.getRawFragment())
    else {
      ...
      new URI(this.getScheme(), this.getAuthority(), path, uri.getRawQuery(), uri.getRawFragment())
        .normalize()
    }
  }
...
object URI {
  ...
  private def uriStr(scheme: String, userInfo: String, host: String, port: Int, path: String,
      query: String, fragment: String): String = {
    var resStr = ""
  ...
  private def uriStr(scheme: String, authority: String, path: String, query: String,
      fragment: String) = {
    var resStr = ""
