...
case class PartestTask(taskDef: TaskDef, args: Array[String]) extends Task {
  ...
  def execute(eventHandler: EventHandler, loggers: Array[Logger]): Array[Task] = {
    ...
    maybeOptions foreach { options =>
      ...
      val runner = SBTRunner(
          ...
          classLoader, null, null, Array.empty[String], testDirs, options,
          scalaVersion)
  ...
  private def SBTRunner(partestFingerprint: Fingerprint,
      eventHandler: EventHandler, loggers: Array[Logger], testRoot: File,
      testClassLoader: URLClassLoader, javaCmd: File, javacCmd: File,
      scalacArgs: Array[String], args: Array[String],
    ...
    runnerClass.getConstructors()(0).newInstance(partestFingerprint,
        eventHandler, loggers, testClassLoader, javaCmd, javacCmd, scalacArgs,
        args, options, scalaVersion).asInstanceOf[SBTRunner]
  }
