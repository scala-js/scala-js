 ...
 * </p>
 * <p>
 * In Scala.js, the client may request multiple instances of <code>Runner</code>, where
 * one of these instances is considered the master. The slaves receive a communication
 * channel to the master. Once the master's <code>done</code> method is invoked, nothing
...
trait Runner {
   ...
   *
   * <p>
   * In Scala.js, the client must not call this method before all execute methods
   * of all Tasks have called their completion continuation. Otherwise, the Framework
   * should throw an IllegalStateException (since it cannot block).
   *
   * Further, if this is the master, the client must not call this method before,
   * all done methods of all slaves have returned (otherwise, IllegalStateException).
   * If this is a slave, the returned string is ignored.
   * </p>
   *
  ...
  /** Scala.js specific:
   *  Invoked on the master <code>Runner</code>, if a slave sends a message
   ...
   *
   *  Invoked on a slave <code>Runner</code>, if the master responds to a message
   *  (sent by the slave via the supplied closure in <code>slaveRunner</code>).
   *  The return value of the call is ignored in this case.
   */
  def receiveMessage(msg: String): Option[String]

  /** Scala.js specific: Serialize a task created by <code>tasks</code> or returned from
   *  <code>execute</code>. The resulting string will be passed to the <code>deserializeTask</code>
  ...
  /** Scala.js specific: Deserialize a task that has been serialized by <code>serializeTask</code>
   *  of another or this <code>Runner</code>. The resulting task must associate with this runner.
   */
