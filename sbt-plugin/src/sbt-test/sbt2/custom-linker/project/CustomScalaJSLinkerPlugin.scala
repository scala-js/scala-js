...
object CustomScalaJSLinkerPlugin extends AutoPlugin {
  ...
  private def scalaJSStageSettings(stage: Stage,
    ...
    Def.settings(
      // Use Def.uncached to avoid needing JsonFormat for ClearableLinker
      ...
      // Use Def.uncached to avoid needing HashWriter for complex types
  ...
  // Convert HashedVirtualFileRef classpath to Files for sbt2
  private def toFiles(cp: Seq[Attributed[HashedVirtualFileRef]])(using conv: FileConverter): Seq[java.io.File] =
    cp.map(a => conv.toPath(a.data).toFile)

  override def globalSettings: Seq[Setting[_]] = Def.settings(
    // Use Def.uncached to avoid needing JsonFormat for LinkerImpl
    scalaJSLinkerImpl := Def.uncached {
      given FileConverter = (ThisBuild / fileConverter).value
      val cp = (scalaJSLinkerImpl / fullClasspath).value
      scalaJSLinkerImplBox.value.ensure {
        new CustomLinkerImpl(LinkerImpl.reflect(toFiles(cp)))
      }
    }
  )
