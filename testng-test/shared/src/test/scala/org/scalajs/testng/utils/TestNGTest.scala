...
abstract class TestNGTest {
  import TestNGTest._

  type Output = TestNGTest.Output
  ...
  private val classLoader: ClassLoader =
    TestNGTestPlatformImpl.getClassLoader
  ...
  @Test def testTestNGOutput(): Unit = {
    for (frameworkArgs <- frameworkArgss) {
      val testNGFramework = new org.scalajs.testng.TestNGFramework()
      val runner =
        testNGFramework.runner(frameworkArgs.toArray, Array.empty, classLoader)
      val tasks = runner.tasks(Array(new TaskDef(suiteUnderTestName,
        testNGFramework.fingerprints.head, true, Array.empty)))

      val recorder: TestNGTestRecorder =
        new TestNGTestRecorder(new OutputContext(frameworkArgs))

      // run all tasks and the tasks they generate, needs platform extension
      TestNGTestPlatformImpl.executeLoop(tasks, recorder)
  ...
  protected class TestNGTestRecorder(context: OutputContext)
