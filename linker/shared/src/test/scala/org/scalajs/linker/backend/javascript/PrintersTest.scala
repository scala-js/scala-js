...
class PrintersTest {
  ...
  @Test def printJSDocConstructor(): Unit = {
    assertPrintEquals(
      ...
      """,
      JSDocConstructor(
          Assign(VarRef("ctor"), Function(ClosureFlags.function, Nil, None, Skip())))
    )
  }
  ...
  @Test def printFor(): Unit = {
    assertPrintEquals(
      ...
      For(Let("x", true, Some(IntLiteral(1))),
          ...
          Assign(VarRef("x"),
              BinaryOp(ir.Trees.JSBinaryOp.+, VarRef("x"), IntLiteral(1))),
          Skip())
    )
  }
  ...
  @Test def showPrintedTree(): Unit = {
    val tree =
      PrintedTree("test".getBytes(UTF_8), SourceMapWriter.Fragment.Empty)
