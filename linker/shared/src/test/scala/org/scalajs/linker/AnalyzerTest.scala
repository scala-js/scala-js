...
class AnalyzerTest {
  ...
  def cycleInInheritanceChainThroughParentClasses(): AsyncResult = await {
    ...
    assertContainsError("CycleInInheritanceChain(A, B)", analysis) {
      case CycleInInheritanceChain(
              List(ClsName("A"), ClsName("B")), `fromAnalyzer`) => true
      case CycleInInheritanceChain(
              List(ClsName("B"), ClsName("A")), `fromAnalyzer`) => true
    }
  }
  ...
  def bigCycleInInheritanceChain(): AsyncResult = await {
    ...
    assertContainsError("CycleInInheritanceChain(C, D, E)", analysis) {
      case CycleInInheritanceChain(
              List(ClsName("C"), ClsName("D"), ClsName("E")), `fromAnalyzer`) =>
        true
      case CycleInInheritanceChain(
              List(ClsName("D"), ClsName("E"), ClsName("C")), `fromAnalyzer`) =>
        true
      case CycleInInheritanceChain(
              List(ClsName("E"), ClsName("C"), ClsName("D")), `fromAnalyzer`) =>
        true
    }
  }
  ...
  def missingClassParentSynthesizingConstructorPlusReflectiveCall_Issue4865(): AsyncResult = {
    await {
      ...
      val requirements = {
        ...
        reqsFactory.callMethod(
            ObjectClass, MethodName.reflectiveProxy("foo", Nil))
      }
      ...
      kindSub match {
        ...
        case HijackedClass =>
          throw new AssertionError(
              "Cannot test HijackedClass because it fails earlier")
      }
    }
  ...
  def missingMethod(): AsyncResult = await {
    ...
    assertContainsError("MissingMethod(A.foo;V)", analysis) {
      case MissingMethod(MethInfo("A", "foo;V"),
              FromDispatch(ClsInfo("A"), `fooMethodName`)) => true
    }
  }
  ...
  def missingInheritedMethod(): AsyncResult = await {
          ...
          methods = List(
            ...
            MethodDef(EMF, barMethodName, NON, Nil, VoidType,
                Some(Block(
                  ...
                  Apply(EAF, New("B", NoArgConstructorName, Nil), fooMethodName,
                      Nil)(VoidType)
                )))(EOH, UNV)
          )),
      classDef("B", superClass = Some("A"),
    ...
    assertContainsError("MissingMethod(B.foo;V) from A", analysis) {
      case MissingMethod(MethInfo("B", "foo;V"),
              FromDispatch(ClsInfo("A"), `fooMethodName`)) => true
    }

    assertContainsError("MissingMethod(B.foo;V) from B", analysis) {
      case MissingMethod(MethInfo("B", "foo;V"),
              FromDispatch(ClsInfo("B"), `fooMethodName`)) => true
    }
  }
  ...
  def missingAbstractMethod(): AsyncResult = await {
    ...
    val classDefs = Seq(
      ...
      classDef("B", superClass = Some("A"),
          methods = List(
            trivialCtor("B", "A"),
            MethodDef(
                EMF, fooMethodName, NON, Nil, IntType, Some(int(5)))(EOH, UNV)
          ))
    )
    ...
    assertContainsError("MissingMethod(A.foo;I)", analysis) {
      case MissingMethod(MethInfo("A", "foo;I"),
              FromDispatch(ClsInfo("A"), `fooMethodName`)) => true
    }
  }
  ...
  def conflictingDefaultMethods(): AsyncResult = await {
    ...
    assertContainsError(
        "ConflictingDefaultMethods(I1.foo;V, I2.foo;V)", analysis) {
  ...
  def invalidTopLevelExportInScript(): AsyncResult = await {
    ...
    testScriptAndModule(classDefs) { scriptAnalysis =>
      assertContainsError(
          "InvalidTopLevelExportInScript(foo, A)", scriptAnalysis) {
        case InvalidTopLevelExportInScript(
                TLEInfo(ModID("main"), "default", ClsName("A"))) =>
          true
      }
    } { moduleAnalysis =>
  ...
  def conflictingTopLevelExportsDifferentModules(): AsyncResult = await {
    ...
    testScriptAndModule(classDefs) { scriptAnalysis =>
      assertContainsError(
          "MultiplePublicModulesWithoutModuleSupport(A, B)", scriptAnalysis) {
        case MultiplePublicModulesWithoutModuleSupport(
                List(ModID("A"), ModID("B"))) =>
          true
        case MultiplePublicModulesWithoutModuleSupport(
                List(ModID("B"), ModID("A"))) =>
          true
      }
    } { moduleAnalysis =>
      assertNoError(moduleAnalysis)
  ...
  def conflictingTopLevelExportsSameModule(): AsyncResult = await {
    ...
    val analysis = computeAnalysis(classDefs)
    assertContainsError(
        "ConflictingTopLevelExport(main, foo, A, B)", analysis) {
  ...
  def degenerateConflictingTopLevelExports(): AsyncResult = await {
    ...
    val analysis = computeAnalysis(classDefs)
    assertContainsError(
        "ConflictingTopLevelExport(_, foo, <degenerate>)", analysis) {
  ...
  def multipleModulesTopLevelExportAndModuleInitializer(): AsyncResult = await {
    ...
    testScriptAndModule(classDefs, moduleInitializers = List(moduleInitializer)) {
      scriptAnalysis =>
        assertContainsError(
            "MultiplePublicModulesWithoutModuleSupport(A, B)", scriptAnalysis) {
          case MultiplePublicModulesWithoutModuleSupport(
    ...
    Future.traverse(kinds) { kind =>
      val classDefs = Seq(
        classDef("A", kind = kind, superClass = Some(ObjectClass),
            jsNativeLoadSpec = Some(
                JSNativeLoadSpec.Import("my-module", List("A"))))
      )
      ...
      assertContainsError(
          "ImportWithoutModuleSupport(my-module, A, None)", analysis) {
        case ImportWithoutModuleSupport(
                "my-module", ClsInfo("A"), None, `fromUnitTest`) =>
          true
      }
    }
  }
  ...
  def asyncWithoutES2017(): AsyncResult = await {
    val classDefs = Seq(
      mainTestClassDef {
        Closure(ClosureFlags.arrow.withAsync(true), Nil, Nil, None, AnyType,
            int(5), Nil)
      }
    )
    ...
    val analysis = computeAnalysis(classDefs,
        moduleInitializers = MainTestModuleInitializers,
        config = StandardConfig().withESFeatures(
            _.withESVersion(ESVersion.ES2016)))
  ...
  def orphanAwaitWithoutWebAssembly(): AsyncResult = await {
    ...
    val analysis = computeAnalysis(classDefs,
        moduleInitializers = MainTestModuleInitializers,
        config = StandardConfig().withESFeatures(
            _.withESVersion(ESVersion.ES2017)))
    ...
    val classDefs = Seq(
      ...
      classDef(
        ...
        methods = List(
          trivialCtor("X"),
          MethodDef(
              EMF, fooAMethodName, NON, Nil, ClassType("A", nullable = true),
              Some(Null()))(EOH, UNV),
          MethodDef(
              EMF, fooBMethodName, NON, Nil, ClassType("B", nullable = true),
              Some(Null()))(EOH, UNV)
        )
      )
    )
    ...
    val classDefs = Seq(
      ...
      classDef("C", superClass = Some("B"),
          methods = List(
            trivialCtor("C", "B"),
            MethodDef(
                EMF, barMethodName, NON, Nil, IntType, Some(int(5)))(EOH, UNV)
          ))
    )
  ...
  def linkTimeIfReachable(): AsyncResult = await {
    ...
    val classDefs = Seq(
      classDef(
        ...
        methods = List(
          trivialCtor("A"),
          MethodDef(
              EMF, mainMethodName, NON, Nil, IntType, Some(mainBody))(EOH, UNV),
          MethodDef(
              EMF, fooMethodName, NON, Nil, IntType, Some(int(1)))(EOH, UNV),
          MethodDef(
              EMF, barMethodName, NON, Nil, IntType, Some(int(2)))(EOH, UNV)
        )
      )
    )
    ...
    val analysisFuture = computeAnalysis(classDefs, requirements,
        config = StandardConfig().withSemantics(
            _.withProductionMode(productionMode)))
  ...
  def linkTimeIfError(): AsyncResult = await {
    ...
    for (analysis <- analysisFuture) yield {
      assertContainsError(
          s"InvalidLinkTimeProperty(core/unknownProperty)", analysis) {
        case InvalidLinkTimeProperty("core/unknownProperty", BooleanType, _) =>
          true
      }
...
object AnalyzerTest {
  ...
  object MethInfo {
    def unapply(methodInfo: Analysis.MethodInfo): Some[(String, String)] =
      Some((methodInfo.owner.className.nameString,
          methodInfo.methodName.nameString))
  }

  object TLEInfo {
    def unapply(tleInfo: Analysis.TopLevelExportInfo): Some[(ModuleID, String,
        ClassName)] =
