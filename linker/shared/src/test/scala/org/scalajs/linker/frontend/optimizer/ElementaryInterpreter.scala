...
/** An IR interpreter for elementary operations, that we use for fuzzing. */
object ElementaryInterpreter {
  ...
  private def eval(tree: Tree)(implicit env: Env): Literal = tree match {
    ...
    case _ =>
      throw new UnsupportedOperationException(
          s"Unsupported tree in the ElementaryInterpreter, " +
          s"of class ${tree.getClass().getSimpleName()}:\n${tree.show}")
  }
    ...
    (op: @switch) match {
      ...
      case _ =>
        throw new UnsupportedOperationException(
            ...
            s"${BinaryOp(op, lhs, rhs).show}")
    }
  }

  private def int(value: Int): IntLiteral = IntLiteral(value)
  ...
  private def long(value: Long): LongLiteral = LongLiteral(value)
