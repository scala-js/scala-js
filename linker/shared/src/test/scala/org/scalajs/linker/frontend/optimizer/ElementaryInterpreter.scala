...
object ElementaryInterpreter {
  ...
  private def eval(tree: Tree)(implicit env: Env): Literal = tree match {
    ...
    // Intercept (expr >>> 0.0d).asInstanceOf[double] to avoid the full semantics of the JS >>>
    case AsInstanceOf(JSBinaryOp(JSBinaryOp.>>>, expr, DoubleLiteral(0.0)), DoubleType)
        if expr.tpe == DoubleType =>
      double(jsToUint32(asDouble(eval(expr))))

    // Intercept (expr | 0.0d).asInstanceOf[int] to avoid the full semantics of the JS |
    case AsInstanceOf(JSBinaryOp(JSBinaryOp.|, expr, DoubleLiteral(0.0)), IntType)
        if expr.tpe == DoubleType =>
      int(jsToInt32(asDouble(eval(expr))))
  ...
  private def findAndEvalClassMethod(ctx: Tree, cls: ClassName,
    ...
    (cls, methodName) match {
      // Intercept RuntimeLong.pack because its IR is fake, and we don't have the Desugarer here
      ...
      // Intercept Integer.divideUnsigned so that we don't need to load the minilib
      ...
      // Same for Integer.remainderUnsigned
  ...
  /** Performs the JS operation `x >>> 0` and returns the result as a Double */
  private def jsToUint32(x: Double): Double =
    Integer.toUnsignedLong(jsToInt32(x)).toDouble
