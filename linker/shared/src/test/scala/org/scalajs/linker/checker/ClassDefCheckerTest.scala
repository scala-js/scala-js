...
class ClassDefCheckerTest {
  ...
  def illegalThisDeclarations(): Unit = {
    ...
    assertError(
      mainTestClassDef(Block(
        Closure(ClosureFlags.arrow, List(thisParamDef), Nil, None, int(5), List(int(6)))
      )),
      "Illegal definition of a variable with name `this`"
    )
    ...
    assertError(
      mainTestClassDef(Block(
        Closure(ClosureFlags.arrow, Nil, List(thisParamDef), None, int(5), Nil)
      )),
      "Illegal definition of a variable with name `this`"
    )
    ...
    assertError(
      mainTestClassDef(Block(
        Closure(ClosureFlags.arrow, Nil, Nil, Some(thisParamDef), int(5), Nil)
      )),
      "Illegal definition of a variable with name `this`"
    )
