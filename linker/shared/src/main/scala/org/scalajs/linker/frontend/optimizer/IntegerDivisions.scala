 ...
 */
private[optimizer] final class IntegerDivisions(
    useRuntimeLong: Boolean, isWebAssembly: Boolean) {
  ...
  def shouldRewriteDivision[T](op: BinaryOp.Code, divisor: T)(
    ...
    val (isSigned, isLong) = op match {
      case BinaryOp.Int_/ | BinaryOp.Int_%                     => (true, false)
      case BinaryOp.Long_/ | BinaryOp.Long_%                   => (true, true)
      case BinaryOp.Int_unsigned_/ | BinaryOp.Int_unsigned_%   => (false, false)
      case BinaryOp.Long_unsigned_/ | BinaryOp.Long_unsigned_% => (false, true)
    }
