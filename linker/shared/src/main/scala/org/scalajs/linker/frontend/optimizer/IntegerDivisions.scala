...
private[optimizer] final class IntegerDivisions(useRuntimeLong: Boolean) {
  ...
  private def genNonPowerOfTwoRuntimeLong(isSigned: Boolean,
    ...
    if (absDivisor < 0L) {
      ...
      makeRTLongApplyStatic(LongImpl.unsignedDivModByConstantHuge,
      ...
      if (isSigned)
        ...
        makeCall(LongImpl.unsignedDivModByConstantSmall, true, mHat)
    } else {
      // Section VII.B
      ...
      if (absDivisor < (1L << 31)) {
        if (isSigned)
          ...
          makeCall(LongImpl.unsignedDivModByConstantMedium, true, mHat)
      } else {
        if (isSigned)
          makeCall(LongImpl.divModByConstantLarge, false, mHat)
        else
          makeCall(LongImpl.unsignedDivModByConstantLarge, false, mHat)
      }
    }
  }
}
