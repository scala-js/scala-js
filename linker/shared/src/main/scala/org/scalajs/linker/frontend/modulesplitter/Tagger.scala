 ...
 *
 *  It is the primary mechanism for the FewestModulesAnalyzer but also used by
 *  the SmallModulesForAnalyzer.
 *
 *  To group classes into modules appropriately, we want to know for each class,
 *  "how" it can be reached. In practice, this means we record the path from the
 *  original public module and every dynamic import hop we made.
 *
 *  Of all these paths, we only care about the "simplest" ones. Or more
 *  formally, the minimum prefixes of all paths. For example, if a class is
 *  reachable by the following paths:
 *
 *    - a -> b
 *    - a -> b -> c
 *    - d -> c
 ...
 *    - d
 *
 *  Because if we reach the class through a path that goes through `c`, it is
 *  necessarily already loaded.
 *
 *  Once we have obtained this minimal set of paths, we use the last element of
 *  each path to determine the final module grouping. This is because these have
 *  an actual static dependency on the node in question.
 *
 *  Merging these tags into a single `ModuleID` is delegated to the caller.
 *
 *  ==Class Exclusion==
 *  Classes can be excluded from the modules generated by Tagger.
 *
 *  For excluded classes, the Tagger assumes that they are in a module provided
 *  by a different part of the overall analysis.
 *
 ...
 *  excessive splitting alike (see #4835), we need to introduce an additonal
 *  tagging mechanism.
 *
 ...
 *  a -> b -> c
 *
 *  where B is excluded. Naively, we would want to group a and c together into
 *  a'. However, this would lead to a circular dependency between a' and c.
 *
 ...
 *  Given
 *
 *  G = (V, E), acyclic, V = F ∪ C, F ∩ C = ∅ the original dependency graph, F:
 *  set of fine classes, C: set of coarse classes
 *
 *  t : V → ℕ (the maxExcludedHopCount tag) ∀ (v1, v2) ∈ E : t(v1) ≤ t(v2) ∀ (f,
 *  c) ∈ E : f ∈ F, c ∈ C ⇒ t(f) < t(c)
 *
 *  Define
 *
 ...
 *  C' = { n ∈ ℕ | ∃ c ∈ C : t(c) = n }
 *
 *  E' = { (f1, f2) ∈ E | f1, f2 ∈ F } ∪ { (f, n) | f ∈ F, ∃ c ∈ C : (f, c) ∈ E
 *  : t(c) = n } ∪ { (n, f) | f ∈ F, ∃ c ∈ C : (c, f) ∈ E : t(c) = n } ∪ { (n,
 *  m) | n ≠ m, ∃ c1, c2 ∈ C : (c1, c2) ∈ E : t(c1) = n, t(c2) = m }
 *
 *  t' : V' → ℕ:
 *
 *  t'(f) = t(f) (if f ∈ F) t'(n) = n (if n ∈ C')
 *
 *  Lemma 1 (unproven)
 *
 *  ∀ (v1, v2) ∈ E' : t'(v1) ≤ t'(v2)
 ...
 *
 *  ∃ xi ∈ F by contradiction: ∀ xi ∈ C' ⇒ t'(xi) increases strictly
 *  monotonically (by Lemma 3), but x1 = xn ⇒ t'(x1) = t'(xn)
 *
 *  Therefore,
 *
 *  ∃ (xi, xj) ∈ p : xi ∈ F, xj ∈ C'
