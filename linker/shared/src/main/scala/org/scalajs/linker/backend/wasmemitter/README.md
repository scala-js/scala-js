...
When using custom descriptors (`wasmFeatures.useCustomDescriptors` set to `true`), the vtables are not stored as user-space fields of the class structs.
...
There must be a 1-1 relationship between descriptor type and described type, and it must hold that `$describedA <: $describedB` iff `$descriptorA <: $descriptorB`.
We naturally have this for all our real vtable types.
However recall that `$v.java.lang.Object` extends `$typeData`, which is therefore a descriptor type.
We introduce `$typeDataDescribed` as a fake type to be the type described by `$typeData`, and `$c.java.lang.Object` extends `$typeDataDescribed`.
That completes the rectangle mandated by the validation rules:

```
 $typeDataDescribed  --described-by-->   $typeData
       ↑                                     ↑
    extends                               extends
       |                                     |
$c.java.lang.Object  --described-by-->  $v.java.lang.Object
```
