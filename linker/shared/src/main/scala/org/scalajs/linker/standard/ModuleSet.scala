 ...
 *      types/members. These are not generated by Scala.js but need to be
 *      provided by the user. For example, the Node.js `fs` module.
 *    - Public Modules: Scala.js-produced modules intended for import by user
 *      code. Only these modules may hold top level exports (but may contain
 *      more). Public modules may not be imported by other Scala.js produced
 *      modules.
 *    - Internal Modules: Scala.js-produced modules only relevant for splitting.
 *      These are only intended to be imported by other Scala.js-produced
 *      modules.
 *
