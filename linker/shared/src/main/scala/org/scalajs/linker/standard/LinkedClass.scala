...
final class LinkedClass(
    // Stuff from Tree
    val name: ClassIdent, val kind: ClassKind, val jsClassCaptures: Option[List[ParamDef]],
    val superClass: Option[ClassIdent], val interfaces: List[ClassIdent],
    val jsSuperClass: Option[Tree], val jsNativeLoadSpec: Option[JSNativeLoadSpec],
    val fields: List[AnyFieldDef], val methods: List[Versioned[MethodDef]],
    val exportedMembers: List[Versioned[JSMethodPropDef]],
    val jsNativeMembers: List[JSNativeMemberDef], val optimizerHints: OptimizerHints,
    val pos: Position,
    // Actual Linking info
    val ancestors: List[ClassName], val hasInstances: Boolean, val hasInstanceTests: Boolean,
    val hasRuntimeTypeInfo: Boolean, val staticDependencies: Set[ClassName],
    val externalDependencies: Set[String], val version: Option[String]) {
  ...
  private[linker] def refined(
      kind: ClassKind, fields: List[AnyFieldDef], methods: List[Versioned[MethodDef]],
      jsNativeMembers: List[JSNativeMemberDef], hasInstances: Boolean, hasInstanceTests: Boolean,
      hasRuntimeTypeInfo: Boolean, staticDependencies: Set[ClassName],
  ...
  private def copy(name: ClassIdent = this.name, kind: ClassKind = this.kind,
      ...
      fields: List[AnyFieldDef] = this.fields, methods: List[Versioned[MethodDef]] = this.methods,
      ...
      optimizerHints: OptimizerHints = this.optimizerHints, pos: Position = this.pos,
      ancestors: List[ClassName] = this.ancestors, hasInstances: Boolean = this.hasInstances,
    ...
    new LinkedClass(name, kind, jsClassCaptures, superClass, interfaces, jsSuperClass,
        jsNativeLoadSpec, fields, methods, exportedMembers, jsNativeMembers, optimizerHints, pos,
        ancestors, hasInstances, hasInstanceTests, hasRuntimeTypeInfo, staticDependencies,
        externalDependencies, version)
  }
}
