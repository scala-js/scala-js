...
private class FunctionEmitter private (
  ...
  private def lookupRecordSelect(tree: RecordSelect): VarStorage = {
    def rec(tree: Tree): VarStorage = tree match {
      case VarRef(ident) =>
      ...
      case _ =>
        throw new AssertionError(s"Unexpected record tree: $tree")
    }
  ...
  def genTree(tree: Tree, expectedType: Type): Unit = {
    val generatedType: Type = tree match {
      ...
      case _: Transient | _: JSSuperConstructorCall =>
