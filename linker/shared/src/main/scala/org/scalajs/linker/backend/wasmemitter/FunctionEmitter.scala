...
private class FunctionEmitter private (
  ...
  def genTree(tree: Tree, expectedType: Type): Unit = {
    /* For JS nodes that accept a castTo parameter, we can always use the
     * expectedType. Indeed, they all declare a `tpe` that is `AnyType`, so the
     * only possible expected types are `AnyType` and `VoidType`, which can be
     * used as the result types of custom JS helpers. It is mostly useful when
     * the expected type is `VoidType`.
     *
  ...
  private def genCast(expr: Tree, targetTpe: Type, pos: Position): Type = {
    ...
    if (sourceTpe == NothingType) {
      ...
      (sourceWasmType, targetWasmType) match {
        ...
        case _ =>
          def targetTpeCastableThroughJSWasmBoundary: Boolean = targetTpe match {
            case UndefType | StringType | CharType | LongType => false
            case _                                            => true
          }
  ...
  private def genJSUnaryOp(tree: JSUnaryOp, castTo: Type): Type = {
    ...
    val resultType = if (castTo == AnyType) tree.tpe else castTo

    genThroughCustomJSHelper(List(lhs), resultType) { allJSArgs =>
  ...
  private def genJSBinaryOp(tree: JSBinaryOp, castTo: Type): Type = {
    ...
    op match {
      ...
      case _ =>
        ...
        genThroughCustomJSHelper(List(lhs, rhs), resultType) { allJSArgs =>
