...
private class FunctionEmitter private (
    ...
    receiver.tpe match {
      ...
      case _ =>
        ...
        BoxedClassToPrimType.get(targetClassName) match {
          case None =>
            genTree(receiver,
                ClassType(targetClassName, nullable = receiver.tpe.isNullable, exact = false))
            genAsNonNullOrNPEFor(receiver)
  ...
  private def getElementaryBinaryOpInstr(op: BinaryOp.Code): wa.Instr = {
    ...
    (op: @switch) match {
      case String_+ =>
        if (ctx.hasJSInterop)
          wa.Call(genFunctionID.stringBuiltins.concat)
        else
          wa.Call(genFunctionID.wasmString.stringConcat)
