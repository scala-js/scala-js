  ...
  private def checkMethodDef(methodDef: MethodDef): Unit = withPerMethodState {
    ...
    for (body <- optBody) {
      val bodyEnv = Env.fromParams(params)
        .withThisTypeUnlessStatic(static, instanceThisType)
      ...
      case Closure(arrow, captureParams, params, restParam, body, captureValues) =>
        ...
        withPerMethodState {
          ...
          val bodyEnv = Env
            ...
            .withThisTypeUnlessStatic(static = arrow, AnyType) // slight abuse of `static`
          checkTree(body, bodyEnv)
        }
...
object ClassDefChecker {
  ...
  private class Env(
    ...
    def withThisTypeUnlessStatic(static: Boolean, thisType: Type): Env =
      if (static) this
      else withThisType(thisType)
