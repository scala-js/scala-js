...
final class CoreWasmLib(coreSpec: CoreSpec, globalInfo: LinkedGlobalInfo) {
  ...
  private def genHelperImports()(implicit ctx: WasmContext): Unit = {
    ...
    addHelperImport(genFunctionID.jsValueToStringCall, List(RefType.any), List(RefType.extern))
    addHelperImport(genFunctionID.jsValueToString, List(anyref), List(RefType.extern))
