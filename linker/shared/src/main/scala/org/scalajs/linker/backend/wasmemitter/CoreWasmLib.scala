...
class CoreWasmLib(coreSpec: CoreSpec) {
   ...
   *  for `value === null`. As implemented, it would return ... if it were.
   */
  private def genValueDescription()(implicit ctx: WasmContext): Unit = {
    ...
    fb.block(anyref) { notOurObjectLabel =>
      fb.block(objectType) { isCharLabel =>
        ...
        fb ++= ctx.getConstantStringInstr("long")
        fb += Return
      }

      // Return the constant string "char"
      fb ++= ctx.getConstantStringInstr("char")
      fb += Return
    }
  ...
  private def genClassCastException()(implicit ctx: WasmContext): Unit = {
    ...
    maybeWrapInUBE(fb, semantics.asInstanceOfs) {
      genNewScalaClass(fb, ClassCastExceptionClass, SpecialNames.StringArgConstructorName) {
        ...
        fb ++= ctx.getConstantStringInstr(" cannot be cast to ")
        fb += Call(genFunctionID.stringConcat)
  ...
  private def genPrimitiveAsInstances()(implicit ctx: WasmContext): Unit = {
    ...
    for (primType <- primTypesWithAsInstances) {
      genPrimitiveAsInstance(primType)
      genBoxedClassAsInstance(primType)
    }
  }

  /** `as.primType: anyref -> prim`. */
  private def genPrimitiveAsInstance(targetTpe: PrimType)(implicit ctx: WasmContext): Unit = {
    val origName = OriginalName("as." + targetTpe.show())

    val resultType = TypeTransformer.transformPrimType(targetTpe)
    ...
    fb.block() { objIsNullLabel =>
      targetTpe match {
        ...
        // For all other types, use type test and unbox, separately
        case _ =>
          fb += LocalGet(objParam)
          fb += BrOnNull(objIsNullLabel)

          // if obj.isInstanceOf[targetTpe]
          targetTpe match {
            case UndefType =>
          ...
          fb.ifThen() {
            ...
            targetTpe match {
              ...
              case targetTpe: PrimTypeWithRef =>
                fb += LocalGet(objParam)
                fb += Call(genFunctionID.unbox(targetTpe.primRef))
            }
            fb += Return
          }
      ...
      fb += GlobalGet(genGlobalID.forVTable(PrimTypeToBoxedClass(targetTpe)))
      fb += Call(genFunctionID.classCastException)
      fb += Unreachable
    }

    // obj is null -- load the zero of the type
    fb += SWasmGen.genZeroOf(targetTpe)

    fb.buildAndAddToModule()
  }

  private def genBoxedClassAsInstance(primType: PrimType)(implicit ctx: WasmContext): Unit = {
    val boxClass = PrimTypeToBoxedClass(primType)
    val targetTpe = ClassType(boxClass)
    ...
    fb.block() { objIsNullLabel =>
      primType match {
        ...
        case CharType | LongType =>
          ...
          fb.block(RefType.anyref) { castFailLabel =>
            fb += LocalGet(objParam)
            fb += BrOnCastFail(castFailLabel, RefType.anyref, RefType.nullable(structTypeID))
            fb += Return
          }

        // For all other types, use type test
        case _ =>
          fb += LocalGet(objParam)
          fb += BrOnNull(objIsNullLabel)
  ...
  private def genBaseArrayAsInstance(baseRef: NonArrayTypeRef)(implicit ctx: WasmContext): Unit = {
    ...
    val fb = newFunctionBuilder(
      genFunctionID.asInstance(irtpe.ArrayType(arrayTypeRef)),
  ...
  private def genThrowModuleInitError()(implicit ctx: WasmContext): Unit = {
    ...
    genNewScalaClass(fb, SpecialNames.UndefinedBehaviorErrorClass,
        SpecialNames.StringArgConstructorName) {
      fb ++= ctx.getConstantStringInstr("Initializer of ")
      fb += LocalGet(typeDataParam)
      fb += Call(genFunctionID.typeDataName)
      fb += Call(genFunctionID.stringConcat)
      fb ++= ctx.getConstantStringInstr(" called before completion of its super constructor")
      fb += Call(genFunctionID.stringConcat)
    }
    fb += ExternConvertAny
    fb += Throw(genTagID.exception)
