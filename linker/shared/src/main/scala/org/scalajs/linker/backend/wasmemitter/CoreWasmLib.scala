...
final class CoreWasmLib(coreSpec: CoreSpec, globalInfo: LinkedGlobalInfo) {
  ...
  val typeDataStructFields: List[StructField] = {
    ...
    val specificArrayVTableHeapType: HeapType =
      if (useCustomDescriptors) HeapType(genTypeID.ObjectArrayVTable, exact = true)
      else HeapType(genTypeID.ObjectVTable, exact = false)
  ...
  private def genBoxedZeroGlobals()(implicit ctx: WasmContext): Unit = {
    ...
    for ((globalID, boxClassName, zeroValueInstr) <- primTypesWithBoxClasses) {
      ...
      val instrs: List[Instr] =
        if (useCustomDescriptors) List(zeroValueInstr, getVTable, StructNew(boxStruct))
  ...
  private def genSpecificArrayTypeData()(implicit ctx: WasmContext): Unit = {
    val typeDataType = RefType(genTypeID.typeData)
    val specificArrayVTableType = RefType(ctx.specificArrayVTableHeapType)
    ...
    fb.loop() { loopLabel =>
      fb.block(specificArrayVTableType) { arrayOfIsNonNullLabel =>
        ...
        fb += RefNull(ctx.specificArrayVTableHeapType) // arrayOf
        ...
        fb += StructNew(ctx.specificArrayVTableTypeID)
