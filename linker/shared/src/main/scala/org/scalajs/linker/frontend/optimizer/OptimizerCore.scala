...
private[optimizer] abstract class OptimizerCore(
  ...
  private def transform(tree: Tree, isStat: Boolean)(
    ...
    val result: Tree = tree match {
      ...
      case ArraySelect(array, index) =>
        ...
        ArraySelect(newArray, transformExpr(index))(makeArraySelectType(newArray.tpe))
        ...
        pretransformExprs(array, index) { (tarray, tindex) =>
          (tarray, tindex) match {
            case (PreTransLocalDef(LocalDef(tpe, /* mutable = */ false,
                ...
                if indexValue >= 0 && indexValue < replacement.elemLocalDefs.size =>
              cont(replacement.elemLocalDefs(indexValue).toPreTransform)

            case _ =>
              val newArray = finishTransformExpr(tarray)
              val newIndex = finishTransformExpr(tindex)
              cont(ArraySelect(newArray, newIndex)(makeArraySelectType(newArray.tpe)).toPreTransform)
          }
        }
    ...
    (intrinsicCode: @switch) match {
      ...
      case LongToString =>
        pretransformApplyStatic(ApplyFlags.empty, LongImpl.RuntimeLongClass,
            MethodIdent(LongImpl.toString_), targs, StringClassType,
            isStat, usePreTransform)(
            cont)
      case LongCompare =>
        pretransformApplyStatic(ApplyFlags.empty, LongImpl.RuntimeLongClass,
            MethodIdent(LongImpl.compare), targs, IntType,
            isStat, usePreTransform)(
            cont)
  ...
  private def makeArraySelectType(arrayType: Type)(implicit pos: Position): Type = {
    arrayType match {
      case NothingType | NullType =>
        /* Will throw / NPE / UB.
         *
         * Note that we cannot easily replace the target ArraySelect with, say a
         * checkNotNullStatement, because it might be used as lhs of an Assign node.
         */
        NothingType

      case arrayType: ArrayType =>
        arrayElemType(arrayType)

      case _ =>
        throw new AssertionError(
            s"got non-array type after transforming ArraySelect at $pos")
    }
  }
