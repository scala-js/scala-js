          ...
          tryOrRollback { cancelFun =>
            pretransformExprs(thenp, elsep) { (tthenp, telsep) =>
              if (shouldExtractBranchToStat(tthenp)) {
                ...
                doExtractBranchToStat(
                    foldUnaryOp(UnaryOp.Boolean_!, tcond), telsep, tthenp)
              } else {
      ...
      case _ =>
        ...
        if (thenp.tpe.base == BooleanType && elsep.tpe.base == BooleanType) {
          (cond, thenp, elsep) match {
            ...
            case (
                    PreTransBinaryOp(
                        op1 @ (Int_== | Int_!= | Int_< | Int_<= | Int_> | Int_>=),
                        l1, r1),
                    PreTransLit(BooleanLiteral(true)),
                    PreTransBinaryOp(
                        op2 @ (Int_== | Int_!= | Int_< | Int_<= | Int_> | Int_>=),
                        l2, r2))
                if isLitOrLocalDef(l1) && isLitOrLocalDef(
                    r1) && l1 == l2 && r1 == r2 =>
              val canBeEqual =
    ...
    (op: @switch) match {
      ...
      case Int_< | Int_<= | Int_> | Int_>= |
        ...
        (lhs, rhs) match {
          ...
          case (IntFlipSign(x), PreTransLit(IntLiteral(r))) =>
            foldBinaryOp(
                otherSignOp, x, PreTransLit(IntLiteral(r ^ Int.MinValue)(rhs.pos)))
            ...
            y match {
              case `opMinValue` =>
                if (signedOp == Int_< || signedOp == Int_>=) {
                  ...
                  foldBinaryOp(
                      if (signedOp == Int_<=) Int_== else Int_!=, lhs, rhs)
                }
              ...
              case _
                  if y == opMinValue + 1 && (signedOp == Int_< || signedOp == Int_>=) =>
                foldBinaryOp(if (signedOp == Int_<) Int_== else Int_!=, lhs,
                    PreTransLit(IntLiteral(opMinValue)))

              case _
                  if y == opMaxValue - 1 && (signedOp == Int_> || signedOp == Int_<=) =>
                foldBinaryOp(if (signedOp == Int_>) Int_== else Int_!=, lhs,
      ...
      case Long_< | Long_<= | Long_> | Long_>= |
        ...
        val (isSigned, otherSignOp, flippedOp, intOp) = (op: @switch) match {
          ...
          case Long_>=         => (true, Long_unsigned_>=, Long_<=, Int_>=)
          case Long_unsigned_< =>
            (false, Long_<, Long_unsigned_>, Int_unsigned_<)
          case Long_unsigned_<= =>
            (false, Long_<=, Long_unsigned_>=, Int_unsigned_<=)
          case Long_unsigned_> =>
            (false, Long_>, Long_unsigned_<, Int_unsigned_>)
        ...
        (lhs, rhs) match {
          ...
          case (LongFlipSign(x), PreTransLit(LongLiteral(r))) =>
            foldBinaryOp(otherSignOp, x,
                PreTransLit(LongLiteral(r ^ Long.MinValue)(rhs.pos)))
          ...
          case (_, PreTransLit(LongLiteral(`opMinValue`))) =>
            if (signedOp == Long_< || signedOp == Long_>=) {
              ...
              foldBinaryOp(
                  if (signedOp == Long_<=) Long_== else Long_!=, lhs, rhs)
            }
