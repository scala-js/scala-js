...
private[optimizer] abstract class OptimizerCore(
    ...
    try {
      try {
        transformMethodDefBody(
            myself, thisType, params, jsClassCaptures, resultType, body, isNoArgCtor)
      } catch {
  ...
  private def transformCapturingBody(captureParams: List[ParamDef],
    ...
    for ((paramDef, tcaptureValue) <- captureParams.zip(tcaptureValues)) {
      ...
      def addCaptureParam(newName: LocalName): LocalDef = {
        ...
        val newParamDef =
          ParamDef(localIdent, newOriginalName, tcaptureValue.tpe.base, mutable)(paramDef.pos)
  ...
  private def pretransformSelectCommon(tree: Select, isLhsOfAssign: Boolean)(
    ...
    pretransformExpr(qualifier) { preTransQual =>
      pretransformSelectCommon(
          tree.tpe, preTransQual, optQualDeclaredType = None, field, isLhsOfAssign)(
          cont)(scope, tree.pos)
    }
  }
      ...
      case _ =>
        ...
        preTransQual.tpe.base match {
          ...
          case ClassType(qualClassName, _, _) if !isLhsOfAssign =>
            if (myself.exists(
                    m => m.enclosingClassName == qualClassName && m.methodName.isConstructor)) {
      ...
      case PreTransLocalDef(localDef) =>
        localDef.replacement match {
          ...
          case InlineClassInstanceReplacement(structure, fieldLocalDefs, cancelFun) =>
            ...
            PreTransRecordTree(
                RecordValue(
                    recordType, structure.fieldNames.map(id => fieldLocalDefs(id).newReplacement)),
                structure, cancelFun)
   ...
   *    inserted.
   */
  private def tryInsertAtFirstEvalContext(valName: LocalName, valTree: Tree, body: Tree): Option[
      Tree] = {
        ...
        if (methodName.isReflectiveProxy || flags.noinline) {
          ...
          if (impls.size == 1) {
            pretransformSingleDispatch(
                flags, impls.head, Some(treceiver), targs, isStat, usePreTransform)(cont) {
              if (isWasm) {
                // Replace by an ApplyStatically to guarantee static dispatch
                val targetClassName = impls.head.enclosingClassName
                val castTReceiver =
                  foldCast(treceiver, ClassType(targetClassName, nullable = true, exact = false))
  ...
  private def pretransformStaticApply(tree: ApplyStatically, isStat: Boolean,
    ...
    if (methodName.isReflectiveProxy) {
      ...
      val target = staticCall(className, MemberNamespace.forNonStaticCall(flags),
          methodName)
      pretransformExprs(receiver, args) { (treceiver, targs) =>
        pretransformSingleDispatch(flags, target, Some(treceiver), targs, isStat, usePreTransform)(
            cont) {
      ...
      case ArrayToJSArray =>
        ...
        tarray match {
          case PreTransLocalDef(
                  LocalDef(_, /* mutable = */ false, replacement: InlineArrayReplacement)) =>
            tryOrRollback { cancelFun =>
              val jsArrayReplacement = InlineJSArrayReplacement(replacement.elemLocalDefs, cancelFun)
              val localDef =
                LocalDef(RefinedType(AnyNotNullType), mutable = false, jsArrayReplacement)
              cont(localDef.toPreTransform)
            } { () =>
              cont(JSArrayConstr(
              ...
              val actualMessage: Tree = if (intrinsicCode == RequireNonNullWithMessage) {
                ...
                trampoline {
                  pretransformApply(ApplyFlags.empty, messageLocalDef.toPreTransform,
                      MethodIdent(SupplierGetMethodName), Nil, StringClassType,
  ...
  private def expandOps(pretrans: PreTransform)(cont: PreTransCont)(
    ...
    def expandLongOpSplit2(methodName: MethodName, tlhs: PreTransform, trhs: PreTransform): TailRec[
        Tree] = {
      withSplitLong(tlhs) { (tlhsLo, tlhsHi, cont1) =>
        withSplitLong(trhs) { (trhsLo, trhsHi, cont2) =>
  ...
  private def foldCast(arg: PreTransform, tpe: Type)(
    ...
    def doCast(tpe: Type): PreTransform = {
      val castTpe = RefinedType(tpe, arg.tpe.allocationSite)
  ...
  private def constrainedLub(lhs: Type, rhs: Type, upperBound: Type, isStat: Boolean): Type = {
    ...
    if (isStat || upperBound == VoidType) {
      ...
      (lhs, rhs) match {
        case (NothingType, _) => rhs
        case (_, NothingType) => lhs

        case (ClassType(lcls, lnullable, lexact), ClassType(rcls, rnullable, rexact))
            if lcls == rcls =>
          ClassType(lcls, lnullable || rnullable, lexact && rexact)
        case (ArrayType(ltypeRef, lnullable, lexact), ArrayType(rtypeRef, rnullable, rexact))
            if ltypeRef == rtypeRef =>
          ArrayType(ltypeRef, lnullable || rnullable, lexact && rexact)
...
private[optimizer] object OptimizerCore {
  ...
  private val JavaScriptExceptionClassType =
    ClassType(JavaScriptExceptionClass, nullable = true, exact = false)
  ...
  private object RefinedType {
    ...
    def apply(tpe: Type): RefinedType =
      RefinedType(tpe, AllocationSite.Anonymous)
  }
   ...
   *  method body.
   */
  private final case class ReplaceWithOtherLocalDef(localDef: LocalDef) extends LocalDefReplacement
  ...
  object MethodAttributes {
    def compute(enclosingClassName: ClassName, methodDef: MethodDef): MethodAttributes = {
      ...
      new MethodAttributes(
          inlineable, shouldInline, isForwarder, jsDynImportInlineTarget, jsDynImportThunkFor)
    }
  }
