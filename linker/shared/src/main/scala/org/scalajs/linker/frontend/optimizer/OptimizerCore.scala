  ...
  private def withNewLocalDef(binding: Binding)(
    ...
    if (value.tpe.isNothingType) {
      ...
      value match {
        ...
        case PreTransCast(PreTransLocalDef(localDef @ LocalDef(_, mutable, replacement)), refinedType)
            if !mutable && refinedType.base != CharType =>
          val newLocalDef = LocalDef(refinedType, mutable, ReplaceWithOtherLocalDef(localDef))
