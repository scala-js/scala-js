...
private[optimizer] abstract class OptimizerCore(
   ...
   *
   *  Unlike arithmetic operations, we normalize literals to the
   *  *right*-hand-side of comparisons. There is no objectively better
   *  direction. Subjectively, I (sjrd) have a hard time reasoning about
   *  comparisons with the constants on the left, so we normalize on the right.
   */
  private def foldCmp(cmp: IntComparison, lhs: PreTransform, rhs: PreTransform)(
    ...
    def booleanLit(value: Boolean): PreTransform =
      PreTransLit(BooleanLiteral(value))

    // Like booleanLit, but preserves the side effects of the operands
  ...
  private def foldTwoIntComparisonsOr(
     ...
     */
    def makeTestForInclusiveRange(lhs: PreTransform, min: Long, max: Long,
        negated: Boolean): PreTransform = {
      val minusOp = if (op1.isLongOp) Long_- else Int_-
      val op = op1.toUnsigned.withRels(if (negated) Rels_> else Rels_<=)
      makeBinaryOp(op, foldBinaryOp(minusOp, lhs, op.makeLit(min)), max - min)
    }

    def forLiterals(lhs: PreTransform, op1: IntComparison, r1: Long,
        op2: IntComparison, r2: Long): PreTransform = {
      if (greaterThan(r1, r2)) {
        // Normalize r1 < r2 according to the chosen signedness
        forLiterals(lhs, op2, r2, op1, r1)
      } else if (r1 == r2) {
        makeBinaryOp(op1 | op2, lhs, r1)
      } else if (op1.rels == Rels_<=) {
        // Since r1 < r2, r1 + 1 won't overflow -- normalize <= r1 to <
        forLiterals(lhs, op1.withRels(Rels_<), r1 + 1L, op2, r2)
      } else if (op2.rels == Rels_>=) {
        // Symmetric
        forLiterals(lhs, op1, r1, op2.withRels(Rels_>), r2 - 1L)
      } else if (op1.hasGT) {
        if (op2.hasLT)
        ...
        if ((op1.hasLT || r1 == combinedOp.minValue) && (op2.hasGT || r2 == combinedOp.maxValue)) {
          ...
          makeTestForInclusiveRange(lhs, r1, r2, negated = false)
        } else {
    ...
    if (lhs1 != lhs2) {
      ...
      (rhs1, rhs2) match {
        case (PreTransLit(r1), PreTransLit(r2)) =>
          forLiterals(lhs1, op1, combinedOp.extractLit(r1), op2, combinedOp.extractLit(r2))
        case _ =>
          combinedInequalitiesOrDefault()
      }
    }
   ...
   *  down combinations of `+`, `-` and `*`. These operations respect modular
   *  arithmetics, which is true for 2^k as much as for 2^32.
   *
...
private[optimizer] object OptimizerCore {
  ...
  private final class IntComparison(private val bits: Int) extends AnyVal {
    ...
    def hasAnyFlag(flags: Int): Boolean = (bits & flags) != 0
    def hasAllFlags(flags: Int): Boolean = (bits & flags) == flags
    def hasFlag(flag: Int): Boolean = hasAnyFlag(flag)

    def isCompatibleWith(that: IntComparison): Boolean = {
      ((this.bits & that.bits & SignednessMask) != 0) &&
    ...
    def toUnsigned: IntComparison =
      new IntComparison((bits & ~SignednessMask) | Unsigned)

    def rels: Int = bits & RelationshipsMask

    def withRels(rels: Int): IntComparison =
      new IntComparison((bits & ~RelationshipsMask) | rels)
    ...
    private def constrainSignedness(bits: Int, constraints: Int): Int =
      bits & (constraints | ~SignednessMask)
    ...
    def |(that: IntComparison): IntComparison =
      new IntComparison(constrainSignedness(this.bits | that.bits, this.bits & that.bits))
     ...
     *
     *  If this operator accepts both signednesses, returns `this`.
     */
    def otherSignedness: IntComparison = {
      val signed = (bits & Unsigned) >>> (UnsignedShift - SignedShift)
      val unsigned = (bits & Signed) << (UnsignedShift - SignedShift)
  ...
  private object IntComparison {
    ...
    final val EQShift = 0
    final val EQ = 1 << EQShift

    final val LTShift = 1
    final val LT = 1 << LTShift

    final val GTShift = 2 // note that `flipped` relies on `GTShift > LTShift`
    final val GT = 1 << GTShift

    final val LongOpShift = 3
    final val LongOp = 1 << LongOpShift

    // We have Signed and Unsigned because == and != can apply to both

    final val SignedShift = 4
    final val Signed = 1 << SignedShift

    final val UnsignedShift = 5 // note that `otherSignedness` relies on `UnsignedShift > SignedShift`
    final val Unsigned = 1 << UnsignedShift

    final val RelationshipsMask = EQ | LT | GT
    final val SignednessMask = Signed | Unsigned
    ...
    def unapply(op: BinaryOp.Code): Option[IntComparison] = (op: @switch) match {
      ...
      case Int_<  => Some(new IntComparison(Signed | Rels_<))
      case Int_<= => Some(new IntComparison(Signed | Rels_<=))
      case Int_>  => Some(new IntComparison(Signed | Rels_>))
      case Int_>= => Some(new IntComparison(Signed | Rels_>=))

      case Long_== => Some(new IntComparison(Signed | Unsigned | LongOp | Rels_==))
      case Long_!= => Some(new IntComparison(Signed | Unsigned | LongOp | Rels_!=))

      case Long_<  => Some(new IntComparison(Signed | LongOp | Rels_<))
      case Long_<= => Some(new IntComparison(Signed | LongOp | Rels_<=))
      case Long_>  => Some(new IntComparison(Signed | LongOp | Rels_>))
      case Long_>= => Some(new IntComparison(Signed | LongOp | Rels_>=))
