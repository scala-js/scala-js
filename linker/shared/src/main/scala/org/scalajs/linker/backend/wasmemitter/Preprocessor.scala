  ...
  private def preprocess(
    ...
    val jsPrototypeHolder: Option[ClassName] = {
      if (!kind.isClass || !clazz.hasInstances ||
        ...
        val holder = {
          if (clazz.exportedMembers.nonEmpty ||
              className == ObjectClass || className == ThrowableClass) {
            className
          } else {
            superClass.get.jsPrototypeHolder.getOrElse {
              throw new AssertionError(s"No JS prototype holder found for ${className.nameString}")
            }
          }
        }
        Some(holder)
