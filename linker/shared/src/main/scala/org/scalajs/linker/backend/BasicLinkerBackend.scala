...
final class BasicLinkerBackend(config: LinkerBackendImpl.Config) extends LinkerBackendImpl(config) {
  ...
  def emit(moduleSet: ModuleSet, output: OutputDirectory, logger: Logger)(
    ...
    logger.timeFuture("BasicBackend: Write result") {
      val writer = OutputWriter.start(output, config.maxConcurrentWrites, skipContentCheck)

      val reports = for (module <- moduleSet.modules) yield {
        val (trees, changed) = emitterResult.body(module.id)
        val report = writeModule(writer, module.id, trees, changed || allChanged)
        if (module.public) List(report) else Nil
      }

      writer.complete().map(_ => new ReportImpl(reports.flatten))
    }.andThen { case _ =>
  ...
  private def writeModule(writer: OutputWriter, moduleID: ModuleID,
      trees: List[js.Tree], changed: Boolean)(implicit ec: ExecutionContext) = {
    ...
    if (config.sourceMap) {
      writer.write(List(jsFileName, sourceMapFileName), changed) { () =>
        writeWithSourceMap(moduleID, cache, trees)
      }
    } else {
      writer.write(jsFileName, changed) { () =>
  ...
  private def writeWithSourceMap(moduleID: ModuleID, cache: PrintedModuleCache,
      trees: List[js.Tree]): List[ByteBuffer] = {
    ...
    List(jsFileWriter.toByteBuffer(), sourceMapWriter.toByteBuffer())
  }
