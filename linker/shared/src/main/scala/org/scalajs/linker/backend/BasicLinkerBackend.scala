...
import org.scalajs.linker.backend.javascript.{ByteArrayWriter, Printers,
  SourceMapWriter, Trees => js}
...
final class BasicLinkerBackend(config: LinkerBackendImpl.Config)
  ...
  private[this] val emitter: Emitter = {
    val prePrinter = {
      if (config.minify) Emitter.PrePrinter.Off
        ...
        new Emitter.PrePrinter.WithSourceMap(fragmentIndex)
  ...
  def emit(moduleSet: ModuleSet, output: OutputDirectory, logger: Logger)(
    ...
    val allChanged0 =
      printedModuleSetCache.updateGlobal(
          emitterResult.header, emitterResult.footer)
    ...
    val writer = new OutputWriter(output, config, skipContentCheck) {
      protected def writeModuleWithoutSourceMap(moduleID: ModuleID,
        ...
        if (force || changed || allChanged) {
          ...
          val jsFileWriter =
            new ByteArrayWriter(sizeHintFor(cache.getPreviousFinalJSFileSize()))
      ...
      protected def writeModuleWithSourceMap(moduleID: ModuleID,
        ...
        if (force || changed || allChanged) {
          ...
          val jsFileWriter =
            new ByteArrayWriter(sizeHintFor(cache.getPreviousFinalJSFileSize()))
          val sourceMapWriter =
            new ByteArrayWriter(sizeHintFor(cache.getPreviousFinalSourceMapSize()))
          ...
          val printer = new Printers.JSTreePrinterWithSourceMap(
              jsFileWriter, smWriter, initIndent = 0)
          for (tree <- trees)
            printer.printStat(tree)
          ...
          cache.recordFinalSizes(
              jsFileWriter.currentSize, sourceMapWriter.currentSize)
...
private object BasicLinkerBackend {
  private final class PrintedModuleSetCache(withSourceMaps: Boolean) {
    ...
    private val modules =
      new java.util.concurrent.ConcurrentHashMap[ModuleID, PrintedModuleCache]
  ...
  private sealed class PrintedModuleCache {
    ...
    def recordFinalSizes(finalJSFileSize: Int,
        finalSourceMapSize: Int): Unit = {
