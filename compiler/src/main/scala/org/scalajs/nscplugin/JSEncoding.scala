...
trait JSEncoding[G <: Global with Singleton] extends SubComponent {
   ...
   *
   *  In practice, this is limited to names that start with a digit. All other
   *  names generated by scalac conform to the identifier names of the IR
   *  (which themselves match `IdentifierName`s from JavaScript).
   *
   *  If a name starts with a digit, it will be prepended with the character
   *  `〇` (Ideographic Number Zero, U+3007, category `LETTER_NUMBER`), which
   *  is a valid first character for an IR/JS identifier name. Since scalac
   *  does not use that character in its names, chained collisions cannot
   *  happen.
   */
  private def mangleJSName(name: String): String = {
    // We test `c <= '9'` first as it is most likely false
    val c = name.charAt(0)
    if (c <= '9' && c >= '0')
      "\u3007" + name
    else
