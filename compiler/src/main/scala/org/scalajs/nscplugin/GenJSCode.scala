      ...
      if (wereRepeated.size > args.size) {
        ...
        for ((arg, wasRepeated) <- args.zipAll(wereRepeated, EmptyTree, false)) yield {
          if (wasRepeated) {
             ...
             * custom `ScalaRunTime.to*VarArgs` method. This preserves the
             * semantic information that it was a varargs, and makes the result
             * type more generic (Seq). That will then allow the optimizer to
             * choose the best possible encoding depending on the target.
             */
            arg match {
