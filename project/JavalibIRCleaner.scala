      ...
      tree.kind match {
        ...
        case JSClass | JSModuleClass =>
          errorManager.reportError(
              s"found non-native JS class ${tree.name.encodedName}")(tree.pos)
      }
    }
  ...
  private final class ClassDefCleaner(enclosingClassName: String,
    ...
    def cleanClassDef(tree: ClassDef): ClassDef = {
      ...
      val preprocessedTree = {
         ...
         */
        val newMemberDefs = memberDefs.filter { m =>
          if (m.name.encodedName == "writeReplace__O") {
            changed = true
            false
          } else {
            true
    ...
    override def transformMemberDef(memberDef: MemberDef): MemberDef = {
      super.transformMemberDef(memberDef) match {
        case m: MethodDef =>
          m.name match {
            case name: Ident =>
              MethodDef(m.flags, transformMethodIdent(name), m.args,
                  m.resultType, m.body)(m.optimizerHints, m.hash)(m.pos)
            case _ =>
              m
          }
        case m =>
    ...
    private def transformMethodIdent(ident: Ident): Ident = {
      ...
      val encodedName = ident.encodedName
    ...
    private def postTransformChecks(classDef: ClassDef): Unit = {
      ...
      for (m <- classDef.memberDefs) {
        if (m.isInstanceOf[MethodDef]) {
          if (!seenMethodNames.add((m.flags.namespace, m.name.encodedName))) {
            reportError(
                s"duplicate method name ${m.name.encodedName} after erasure")(
                m.pos)
          }
        }
      }
