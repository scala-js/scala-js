...
object Math extends js.Object {
   ...
   *
   *  The asin method returns a numeric value between -pi/2 and pi/2 radians for
   ...
   *
   *  If at least one of arguments cannot be converted to a number, the result
   *  is NaN.
   *
   *  MDN
   */
   ...
   *  in the range [0, 1) that is, from 0 (inclusive) up to but not including 1
   ...
   *
   *  @return
   *    The natural logarithm (base e) of 1 plus the given number. If the number
   *    is less than -1, NaN is returned. MDN
   */
  ...
  /** <span class="badge badge-ecma6" style="float: right;">ECMAScript 6</span>
   *  The Math.expm1() function returns e^x - 1, where x is the argument, and e
   *  the base of the natural logarithms.
   *
   *  @return
   *    A number representing e^x - 1, where e is Euler's number and x is the
   *    argument.
   */
