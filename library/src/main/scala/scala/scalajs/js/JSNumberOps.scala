...
trait JSNumberOps extends js.Any {
   ...
   *  1e+21, this method simply calls Number.prototype.toString() and returns a
  ...
  /** Returns a string representing a Number object in exponential notation with
   *  one digit before the decimal point, rounded to fractionDigits digits after
   *  the decimal point. If the fractionDigits argument is omitted, the number
   *  of digits after the decimal point defaults to the number of digits
   ...
   *  fractionDigits digits. See the discussion of rounding in the description
   *  of the toFixed() method, which also applies to toExponential().
   *
   *  MDN
   */
   ...
   *  Number.prototype.toFixed() method, which also applies to toPrecision.
   *
   *  If the precision argument is omitted, behaves as
   *  Number.prototype.toString(). If it is a non-integer value, it is rounded
