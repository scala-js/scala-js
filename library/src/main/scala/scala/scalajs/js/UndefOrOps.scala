...
final class UndefOrOps[A] private[js] (private val self: js.UndefOr[A])
   ...
   *  @example
   *    {{{ val initalText: Option[String] = getInitialText val textField = new
   *    JComponent(initalText.orNull,20) }}}
   */
   ...
   *  is nonempty. Otherwise return $none.
   *
   *  @note
  ...
  /** Returns this $option if it is nonempty '''and''' applying the predicate
   *  `p` to this $option's value returns true. Otherwise, return $none.
   *
   *  @param p
  ...
  /** Returns this $option if it is nonempty '''and''' applying the predicate
   *  `p` to this $option's value returns false. Otherwise, return $none.
   *
   *  @param p
   *    the predicate used for testing.
   */
   ...
   *
   *  `x.contains(y)` differs from `x == y` only in the fact that it will return
   *  `false` when `x` and `y` are both `undefined`.
   *
   *  @param elem
   *    the element to test.
   *  @return
  ...
  /** Returns true if this option is empty '''or''' the predicate `p` returns
  ...
  /** Returns the result of applying `pf` to this $option's contained value,
   *  '''if''' this option is nonempty '''and''' `pf` is defined for that value.
  ...
  /** Returns a `Some` containing this $option's value if this $option is
   *  nonempty, [[scala.None None]] otherwise.
   */
