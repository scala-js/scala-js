 ...
 *  constructor was called. This is different from using eval with code for a
 *  function expression.
 *
...
class Function(args: String*) extends js.Object {
   ...
   *  arguments the function expects, i.e. the number of formal parameters. This
   *  number does not include the rest parameter. By contrast, arguments.length
   *  is local to a function and provides the number of arguments actually
   ...
   *  parameters. With apply, you can use an array literal, for example,
   *
