...
trait JSStringOps extends js.Any {
   ...
   *  slower execution) use match (similar to the regular expression exec
   *  method).
   *
   *  MDN
   */
   ...
   *  extracts the second character through the fourth character (characters
   *  indexed 1, 2, and 3).
   *
  ...
  /** Returns a subset of a string between one index and another, or through the
   *  end of the string.
   *
   *  MDN
   */
   ...
   *  cases, this will produce the same result as toLowerCase(), but for some
   *  locales, such as Turkish, whose case mappings do not follow the default
   *  case mappings in Unicode, there may be a different result.
   *
   *  MDN
   */
   ...
   *  upper case according to any locale-specific case mappings.
   *  toLocaleUpperCase does not affect the value of the string itself. In most
   *  cases, this will produce the same result as toUpperCase(), but for some
   *  locales, such as Turkish, whose case mappings do not follow the default
   *  case mappings in Unicode, there may be a different result.
   *
   *  MDN
