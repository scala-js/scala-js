 ...
 *
 *  The methods exposed by this object are not standard in ECMAScript. Different
 *  JavaScript environments support all, some or none of them.
 *
...
object RawTimers extends js.Object {
   ...
   *  @return
   *    A handle that can be used to cancel the timeout by passing it to
   *    [[clearTimeout]].
   */
  def setTimeout(handler: js.Function0[Any],
   ...
   *  @return
   *    A handle that can be used to cancel the interval by passing it to
   *    [[clearInterval]].
   */
  def setInterval(handler: js.Function0[Any],
      interval: Double): SetIntervalHandle = js.native
