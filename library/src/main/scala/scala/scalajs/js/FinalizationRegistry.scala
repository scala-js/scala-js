...
/** <span class="badge badge-ecma2021" style="float: right;">ECMAScript
 *  2021</span>
 *
 *  A FinalizationRegistry object lets you request a callback when an object is
 *  garbage-collected.
 *
...
@js.native
@JSGlobal
class FinalizationRegistry[-A, -B, -C](finalizer: js.Function1[B, scala.Any])
    extends js.Object {

  /** The `register` method registers an object with a FinalizationRegistry
   *  instance so that if the object is garbage-collected, the registry's
   *  callback may get called.
   *
   *  @param theObject
   ...
   *    unregister the target object. If provided (and not `undefined`), this
   ...
   *  registry. There is no need to all `unregister` in your cleanup callback.
   *  Only call `unregister` if you haven't received a cleanup callback and no
   *  longer need to receive one.
   *
   *  @param unregistrationToken
   *    The token used with the register method when registering the target
   *    object.
   */
