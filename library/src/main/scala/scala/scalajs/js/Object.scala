...
object Object extends js.Object {
   ...
   *  from a source object to a target object. It uses `Get` on the source and
   *  `Set` on the target, so it will invoke getters and setters. Therefore it
   *  assigns properties versus just copying or defining new properties. This
   *  may make it unsuitable for merging new properties into a prototype if the
   *  merge sources contain getters. For copying property definitions, including
   *  their enumerability, into prototypes `Object.getOwnPropertyDescriptor()`
   ...
   *  present by dint of being along an object's prototype chain) of a given
   *  object.
   *
   *  MDN
   */
   ...
   *  is consistent with the ordering exposed by a for...in loop (or by
   *  Object.keys) over the properties of the object. The ordering of the
   *  non-enumerable properties in the array, and among the enumerable
   ...
   *  which show up during property enumeration (for...in loop or Object.keys
   *  method), whose values may be changed, and which may be deleted. This
   *  method allows these extra details to be changed from their defaults.
   *
   ...
   *  property described by a getter-setter pair of functions. A descriptor must
   *  be one of these two flavors; it cannot be both.
   *
   *  MDN
   */
   ...
   *  configurability, or writability, from being changed. In essence the object
   *  is made effectively immutable. The method returns the object being frozen.
   *
   *  MDN
   */
   ...
   *  preventExtensions marks an object as no longer extensible, so that it will
   *  never have properties beyond the ones it had at the time it was marked as
   *  non-extensible. Note that the properties of a non-extensible object, in
   *  general, may still be deleted. Attempting to add new properties to a
   *  non-extensible object will fail, either silently or by throwing a
   ...
   *
   *  <ul> <li>both undefined <li>both null <li>both true or both false <li>both
   *  strings of the same length with the same characters in the same order
   *  <li>both the same object (means both object have same reference) <li>both
   *  numbers and <ul> <li>both +0 <li>both -0 <li>both NaN <li>or both non-zero
   *  and both not NaN and both have the same value </ul> </li> </ul>
   *
   ...
   *  them, and (in engines that support __proto__ their __proto__ property) can
   *  be modified. An object can be marked as non-extensible using
