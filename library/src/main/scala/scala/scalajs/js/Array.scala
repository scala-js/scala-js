 ...
 *  MDN
 *
 *  To construct a new array with uninitialized elements, use the constructor of
 *  this class. To construct a new array with specified elements, as if you used
 *  the array literal syntax in JavaScript, use the [[Array$.apply Array.apply]]
 *  method instead.
 *
 *  @tparam A
...
class Array[A] extends js.Object with js.Iterable[A] {
  ...
  /** Sets the length of the array. If the new length is bigger than the old
   *  length, created slots are filled with `undefined` (irrespective of the
   *  type argument `A`!). If the new length is smaller than the old length, the
   *  array is shrunk.
   */
